...
func linkFixDR() string {
	...
	dr := byteme("wp", "search-replace", "--url="+destURL+"/"+siteName, "--all-tables-with-prefix", sourceURL, destURL, "--dry-run")
	return string(dr)
}
...
func assetCopyDR(sid, did string) string {
	...
	dr := byteme("rsync", "-a", "/data/www-assets/"+sourcePath+"/uploads/sites/"+sid+"/", "/data/www-assets/"+destPath+"/uploads/sites/"+did+"/", "--stats", "--dry-run")
	return string(dr)
}
...
func uploadsFolderDR(sid, did string) string {
	...
	dr := byteme("wp", "search-replace", "--url="+destURL+"/"+siteName, "--all-tables-with-prefix", "app/uploads/sites/"+sid, "app/uploads/sites/"+did, "--dry-run")
	return string(dr)
}
...
func uploadsFolderEscapesDR(sid, did string) string {
	...
	dr := byteme("wp", "search-replace", "--url="+destURL, "--all-tables-with-prefix", "app\\/uploads\\/sites\\/"+sid, "app\\/uploads\\/sites\\/"+did, "--dry-run")
	return string(dr)
}
...
func httpFindDR() string {
	...
	dr := byteme("wp", "search-replace", "--url="+destURL+"/"+siteName, "--all-tables-with-prefix", "http://", "https://", "--dry-run")
	return string(dr)
}
